...
from keras.preprocessing.image import \
    ImageDataGenerator  # Nos ayuda a preprocesar las imagenes que le entreguemos al modelo
...
datos_entrenamiento = '../resources/training'
datos_validacion = '../resources/validation'
...
clases = 4  # Mano abierta y cerrada (5 dedos y 0 dedos)
...
cnn.add(Convolution2D(filtrosconv1, tam_filtro1, padding='same', input_shape=(altura, longitud, 3),
                      activation='relu'))  # Agregamos la primera capa
# Es una convolucion y realizamos config
cnn.add(MaxPooling2D(
    pool_size=tam_pool))  # Despues de la primera capa vamos a tener una capa de max pooling y asignamos el tamaño
...
cnn.add(Dense(clases,
              activation='softmax'))  # Es nuestra ultima capa, es la que nos dice la probabilidad de que sea alguna de las clases
...
cnn.fit(imagen_entreno, steps_per_epoch=pasos, epochs=iteraciones, validation_data=imagen_validacion,
        validation_steps=pasos_validacion)

# Guardamos el modelo
cnn.save('model1.h5')
cnn.save_weights('weight1.h5')
